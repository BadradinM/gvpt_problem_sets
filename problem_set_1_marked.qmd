...
::: {.callout-tip title="HG comments"}
Be a touch more careful with your rounding. For example:

```{r}
library(dplyr)
library(poliscidata)
janitor::tabyl(gss, partyid_3) |> 
...
::: {.callout-tip title="HG comments"}
You are correct that there are no outliers within these data; however, we need you to add more on how you know that. What is a good method for detecting outliers? You can look at data points that fall outside of 1.5 times the interquartile range from the median. A boxplot will display this range for you.
:::

::: {.callout-tip title="HG comments"}
The data are not skewed to the left. You can use a smaller bin size to look at the distribution in more detail. In fact, the data are technically slightly skewed to the right. For right skewed data, the mean is greater than the median. For this group, the mean IQ is `r round(mean(iq), 1)` and the median IQ is `r median(iq)`; however, this difference is so small that I would say this is approximately normal (or symmetrical).
